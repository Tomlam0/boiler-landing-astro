...
function MotionPreset({
  ...
  // Get the motion component dynamically - use unknown as intermediate type
  const motionObj = motion as unknown as Record<
    string,
    React.ComponentType<Record<string, unknown>>
  >;
  const MotionComponent = motionObj[component];
  ...
  const mergedAnimate = isInView
    ? motionAnimate
      ? { ...visibleVariant, ...(motionAnimate as Record<string, unknown>) }
      : 'visible'
    : 'hidden';

  // Merge transition: motionTransition takes precedence for continuous animations
  const mergedTransition = motionTransition
    ? motionTransition
    : {
        ...transition,
  ...
  return (
    <AnimatePresence>
      <MotionComponent
        ...
        transition={mergedTransition as Transition}
        className={className}
        {...restMotionProps}
      >
        {children}
